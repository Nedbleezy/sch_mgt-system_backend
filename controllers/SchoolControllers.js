...
export const schoolReg = async (req, res) => {
  try {
    ...
    if (!schoolAlreadyRegistered) {
      ...
      const emailsent = await SendEmail(school.email, school.schoolName, link); //check again why error handling is not working

      if (emailsent.message === 'Email sent successfully') {
        return res
          .status(200)
